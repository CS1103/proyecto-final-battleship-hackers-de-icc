...
int rand_int(int first, int last) {
    ...
    std::uniform_int_distribution<int> gen{ first, last };
    return gen(eng);
}

location_t get_coordenate(std::string value)
{
    std::regex r(R"(^([A-Z])(\d+)$)");
    ...
    size_t ucol_ = col_[0] - 'A';
    size_t urow_ = std::stoi(row_);
...
void controller_t::load_tokens()
{
    ...
    while (true) {
        try {
            filesystem::directory_iterator first_{ players_[0]->get_path() / "in" };
            ...
            while (first_ != end_ || second_ != end_) {
                ...
                if (second_ != end_) {
                    statements_.push({ 1, push_statement(*second_) });
