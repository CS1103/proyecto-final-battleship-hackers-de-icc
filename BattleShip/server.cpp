...
bool server_t::is_model_full(const model_t& model) {
    return (fleet_size_[model] + 1 > predefined_fleet[model].first);
}

bool server_t::is_navy_outside(const rectangle_t& rect, const rectangle_t& battle_field)
{
    auto [loc, dim] = rect;
    auto x1 = loc.x;
    auto y1 = loc.y;
...
bool server_t::is_navy_overlap(const rectangle_t& rect)
{
...
status_t server_t::add_navy(const location_t& location, const model_t& model,
