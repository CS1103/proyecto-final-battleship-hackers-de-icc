...
statement_t player_t::push_statement(path_t file_name)
{
...
void player_t::start(statement_item_t item)
{
    ...
    size_t token_ = player_->get_token();
    text_t scope_ = columns_ + "-" + std::to_string(rows_);

    auto file_name_ = server_->get_prefix() + std::to_string(player_->sequence()++) + ".in";
    std::ofstream file(server_->get_path() / "in" / file_name_);
    file << "TOKEN=" << token_ << "\n";
    file << action_ << "\n";
}

void player_t::build(statement_item_t item)
{
...
void player_t::attack(statement_item_t item)
{
    auto& player_ = players_[item.first];
...
player_t::player_t(size_t rows, std::string_view columns,
                   std::string_view first, std::string_view second) : rows_{rows}, columns_{columns}
{
        if (!filesystem::exists(server_->get_path() / "out"))
            filesystem::create_directories(server_->get_path() / "out");
        if (!filesystem::exists(server_->get_path() / "in"))
            filesystem::create_directories(server_->get_path() / "in");
    }
void player_t::execute()
{
